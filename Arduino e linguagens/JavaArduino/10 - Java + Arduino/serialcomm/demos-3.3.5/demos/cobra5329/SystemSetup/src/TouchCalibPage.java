...
public class TouchCalibPage extends Container {
	...
	private TouchThread m_touchThread;
	...
	class TouchThread extends Thread {
		...
		public void run() {
			try {
				while(m_polling) {
					ThreadExt.sleep(1);
					boolean isPressed = SystemSetup.pointingDevice.isPressed();
					if (!m_wasPressed && isPressed) {
					    ...
					    d.fillOval(SystemSetup.pointingDevice.getX()-3,SystemSetup.pointingDevice.getY()-3,6,6);				
